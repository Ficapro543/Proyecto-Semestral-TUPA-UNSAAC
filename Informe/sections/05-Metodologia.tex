...
La elección de Scrum se fundamenta en las siguientes razones:

\begin{itemize}
      \item El proyecto tiene un alcance acotado al semestre académico, con fechas de entrega
      definidas, lo que es directamente compatible con la planificación en sprints de
      duración fija.
      \item El equipo es pequeño (dos a tres integrantes), lo que facilita la autogestión, la
      comunicación directa y la toma de decisiones sin estructuras jerárquicas complejas.
      ...
      \item Scrum permite alinear cada sprint con los entregables formales del curso, haciendo
      la metodología coherente con el calendario académico establecido.
      \item Se definen tres sprints principales, de aproximadamente tres a cinco semanas de
      duración cada uno, articulados con los tres entregables semestrales del proyecto.
\end{itemize}
...
La elección del stack tecnológico se basa en la necesidad de construir una aplicación
web moderna, escalable y mantenible, que permita una experiencia de usuario fluida y
eficiente. Además, del análisis de los sistemas de referencia (PLADDES-UNSAAC, PUCP, UNH)
en los cuales se identificaron tecnologías como PHP, Java~EE, WordPress y SAP, se opta 
por un stack más contemporáneo y orientado a la modularidad:
\begin{itemize}
  \item \textbf{PLADDES -- UNSAAC} (\texttt{tramite.unsaac.edu.pe}): sistema actual
    ...
    % ── AÑADIDO ──
    En cuanto a su stack tecnológico, el panel administrativo interno está
    construido sobre la plantilla \textbf{AdminLTE} (Bootstrap~3 + jQuery),
    el backend emplea \textbf{PHP con CodeIgniter} siguiendo el patrón MVC
    con enrutamiento limpio, la autenticación integra \textbf{Google OAuth2},
    los iconos del formulario público corresponden a \textbf{Material Design
    Icons} y la persistencia se gestiona con \textbf{MySQL}.
  ...
  \item \textbf{Portal Administrativo -- PUCP}: plataforma que centraliza
    ...
    % ── AÑADIDO ──
    Su stack es heterogéneo: el portal informativo
    (\texttt{administrativo.pucp.edu.pe}) está construido sobre \textbf{WordPress}
    (PHP~CMS), evidenciado por las rutas \texttt{wp-content/uploads/} en sus
    recursos estáticos; mientras que el sistema académico-administrativo central
    (Campus Virtual / Intranet) integra \textbf{SAP} para la gestión de finanzas,
    presupuestos y recursos humanos, lo que revela una arquitectura empresarial
    de dos capas con CMS público y ERP corporativo en el backend.
  ...
  \item \textbf{SISADES -- Universidad Nacional de Huancavelica}: sistema
    ...
    % ── AÑADIDO ──
    El Sistema de Gestión Documentaria (SGD) activo,
    accesible en \texttt{sgd.unh.edu.pe:8181/mpd/inicio.do}, expone un stack
    \textbf{Java~EE}: el puerto 8181 es indicativo de \textbf{Apache Tomcat}
    como servidor de aplicaciones y la extensión \texttt{.do} en las URLs es
    característica de \textbf{Apache Struts} o \textbf{Spring~MVC}; la
    plataforma fue desarrollada y desplegada por la Oficina de Tecnología
    de la Información de la UNH en 2025 como reemplazo de la plataforma
    Facilita~Perú.
\end{itemize}
